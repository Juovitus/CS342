...
public interface Value {
	...
	static class PairVal implements Value {
		...
		public boolean listEquals(PairVal comp) {
			ArrayList<String> s = new ArrayList<>();
			s.add(_fst.tostring());
			Value next = _snd;
			while(!(next instanceof Value.Null)) {
				s.add(((PairVal)next)._fst.tostring());
				next = ((PairVal) next)._snd;
			}

			if(s.contains(comp._fst.tostring())) {
				s.remove(comp._fst.tostring());
			}else{
				return false;
			}
			next = comp._snd;
			while(!(next instanceof Value.Null)) {
				if(s.contains(((PairVal)next)._fst.tostring())) {
					s.remove(((PairVal)next)._fst.tostring());
				}else{
					return false;
				}
				next = ((PairVal) next)._snd;
		...
		public double compare(PairVal comp) {
			double myLength = 0;
			double compLength = 0;
			ArrayList<String> s = new ArrayList<>();
			myLength++;
			...
			return Double.compare(myLength, compLength);
		}
	}
