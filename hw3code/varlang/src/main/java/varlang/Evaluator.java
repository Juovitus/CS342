...
import varlang.Env.AngyEnv;
import varlang.Env.EncEnv;
import varlang.Env.DecEnv;
...
public class Evaluator implements Visitor<Value> {
	...
	@Override
	public Value visit(UniqueExp e, Env env) { // New for varlang.
		List<String> names = e.names();
		//Check for dupes?
		int f = 1;
		for(int i = 0; i < names.size(); i++){
			for(int n = i + 1; n < names.size(); n++){
				if(names.get(i).equals(names.get(n))){
					Env new_env = env;
					new_env = new AngyEnv("Duplicate Variables");
				}
			}
		}
	...
	public Value visit(LeteExp e, Env env) {
		...
		List<Exp> value_exps = e.value_exps();
		List<Value> values = new ArrayList<Value>(value_exps.size());
		Value enc = (Value) e._enc.accept(this, env);
		...
		Env new_env = env;
		for (int i = 0; i < names.size(); i++)
			new_env = new EncEnv(new_env, names.get(i), values.get(i), enc);
		return (Value) e.body().accept(this, new_env);
	}

	@Override
	public Value visit(DecExp d, Env env) {
		Value valueDecoder = (Value) d._decoder.accept(this, env);
		NumVal numValDecoder = (NumVal) d.decoder().accept(this, env);
		NumVal result = new NumVal(((NumVal)valueDecoder).v());
		return new NumVal(result.v() - numValDecoder.v());
	}
}
