...
Q4
a. if e1 or e2 type is errort then return errort
    if e1 or e2's type is numT and the other e's type is not then return errort with statement saying what
        doesn't match
    if none of those are true then return BoolT
b. Type rule to compare two lists would be compare list size to list size, or list size to num, otherwise
    return boolT. If one is not of numT(list size or num) then return errorT with statement saying which
    expression is not of correct type.
Q5
a.  If e1 is numType and e2 is Numtype && e2 is == 0 then return new ErrorT showing where divide by 0 is error
