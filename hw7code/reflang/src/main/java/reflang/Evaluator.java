...
public class Evaluator implements Visitor<Value> {
	...
	public Value visit(DerefExp e, Env env) { // New for reflang
		...
		VarExp f = (VarExp) loc_exp;
		System.out.println("Variable Freed : name " + f._name + " ref value " +  loc.tostring());
	...
	public Value visit(AssignExp e, Env env) { // New for reflang
		...
		if(rhs_val.getClass().isAssignableFrom(heap.deref(loc).getClass())){
			Value assign_val = heap.setref(loc, rhs_val);
			return assign_val;
		}else{
			return new Value.DynamicError("Assigning a value of an incompatible type to the location in " + ts.visit(e, null).toString());
		}
	}
	...
	public Value visit(FreeExp e, Env env) { // New for reflang.
		Exp value_exp = e.value_exp();
		VarExp f = (VarExp) value_exp;
		try{
			Value.RefVal loc = (Value.RefVal) value_exp.accept(this, env);
			if(heap.free(loc) != null){
				System.out.println("Variable Freed : name " + f._name + " ref value " +  loc.tostring());
			}

		}catch (Exception ee){
			System.out.println("Free-ing before setting.");
		}
	...
	public Value visit(RefHashtagExp e, Env env) {
		Exp val = e.RefHashtagExp();
		Value val2 = (Value) val.accept(this, env);
		if(val2 instanceof RefVal){
			return new BoolVal(true);
		}
		return new BoolVal(false);
	}
